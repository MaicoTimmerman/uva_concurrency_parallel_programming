...
program. This time it will be parallelized using OpenMPI. This means that
message passing has to be used to communicate between different instances of the
program instead of just being able to write to the same memory space, because
each node has its own memory and accessing another node's memory is relatively
slow. So the transfer of data is kept to a minimum, and we will use messages to
...
completed, all instances will send their finished wave calculation to the
controlling process, which will write the results to a file.

\section{Results}
Due to struggles during programming this exercise we were not able to run
the program on the DAS4, therefor there are no results of this assignment.

\section{Discussion}
Since there are no results, a conclusion could not be made.
